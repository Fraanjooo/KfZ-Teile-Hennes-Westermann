...
const Editor = () => {
  ...
  const generateSlug = (title: string) => {
    return title.toLowerCase().replace(/ä/g, 'ae').replace(/ö/g, 'oe').replace(/ü/g, 'ue').replace(/ß/g, 'ss').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
  };
  ...
  return <div className="min-h-screen bg-blog-background p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/admin/blog">
              <Button variant="outline" size="sm" className="border-blog-accent text-blog-accent hover:bg-blog-accent hover:text-white">
          ...
          <div className="flex items-center gap-4">
            {lastSaved && <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            {autoSaving && <span className="text-sm text-muted-foreground">Speichert...</span>}
          </div>
        </div>

        {notification && <NotificationBanner type={notification.type} message={notification.message} className="mb-6" />}

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-2 mb-4 bg-blog-badge-bg">
                <TabsTrigger value="edit" className="flex items-center gap-2 data-[state=active]:bg-blog-accent data-[state=active]:text-white">
                  <Edit className="h-4 w-4" />
                  Bearbeiten
                </TabsTrigger>
                <TabsTrigger value="preview" className="flex items-center gap-2 data-[state=active]:bg-blog-accent data-[state=active]:text-white">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    <div>
                      ...
                      <Input id="title" value={formData.title} onChange={e => handleTitleChange(e.target.value)} placeholder="Artikeltitel" />
                    </div>
                    ...
                    <div>
                      ...
                      <Input id="author" value={formData.author} onChange={e => setFormData(prev => ({
                      ...prev,
                      author: e.target.value
                    }))} placeholder="Autorname" />
                      {!formData.author && <p className="text-xs text-yellow-600 mt-1">⚠️ Bitte ergänzen</p>}
                    </div>

                    <ImageUploader imageUrl={formData.featuredImageUrl} imageAlt={formData.featuredImageAlt} seoImageTag={formData.seoImageTag} onImageUrlChange={url => setFormData(prev => ({
                    ...prev,
                    featuredImageUrl: url
                  }))} onImageAltChange={alt => setFormData(prev => ({
                    ...prev,
                    featuredImageAlt: alt
                  }))} onSeoImageTagChange={tag => setFormData(prev => ({
                    ...prev,
                    seoImageTag: tag
                  }))} />
                    ...
                    }))} rows={3} placeholder="Kurze Zusammenfassung für die Übersicht" />
                    </div>

                    <div>
                      <Label>Inhalt *</Label>
                      {activeTab === "edit" && <RichTextEditor value={formData.content} onChange={value => setFormData(prev => ({
                      ...prev,
                      content: value
                    }))} />}
                    </div>
                    ...
                    <div>
                      ...
                      <Input id="tags" value={formData.tags} onChange={e => setFormData(prev => ({
                      ...prev,
                      tags: e.target.value
                    ...
                    <div>
                      ...
                      <Input id="readTime" type="number" value={formData.readTimeMinutes} onChange={e => setFormData(prev => ({
                      ...prev,
                ...
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    <div>
                      ...
                      <Input id="seoTitle" value={formData.seoTitle} onChange={e => setFormData(prev => ({
                      ...prev,
                      seoTitle: e.target.value
                    }))} placeholder="Optimierter Titel für Suchmaschinen" maxLength={60} />
                      <p className="text-xs text-muted-foreground mt-1">
                        {formData.seoTitle.length}/60 Zeichen
                      </p>
                      {!formData.seoTitle && <p className="text-xs text-yellow-600 mt-1">⚠️ Bitte ergänzen</p>}
                    </div>

                    <div>
                      <Label htmlFor="metaDesc">Meta Description (120-160 Zeichen)</Label>
                      <Textarea id="metaDesc" value={formData.metaDescription} onChange={e => setFormData(prev => ({
                      ...prev,
                      metaDescription: e.target.value
                    }))} rows={3} placeholder="Beschreibung für Suchergebnisse" maxLength={160} />
                      <p className="text-xs text-muted-foreground mt-1">
                        {formData.metaDescription.length}/160 Zeichen
                      </p>
                      {!formData.metaDescription && <p className="text-xs text-yellow-600 mt-1">⚠️ Bitte ergänzen</p>}
                    </div>

                    <div>
                      <Label htmlFor="keywords">SEO Keywords (mind. 3)</Label>
                      <Input id="keywords" value={formData.seoKeywords} onChange={e => setFormData(prev => ({
                      ...prev,
              ...
              <TabsContent value="preview">
                <Card>
                  <CardContent className="p-0">
                    <ArticlePreview title={formData.title} author={formData.author} content={formData.content} featuredImageUrl={formData.featuredImageUrl} featuredImageAlt={formData.featuredImageAlt} />
                  </CardContent>
                </Card>
              </TabsContent>
            </Tabs>
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={handleSave} disabled={saving} className="w-full bg-blog-accent hover:bg-blog-accent-hover text-white">
            ...
            <SeoChecker title={formData.title} seoTitle={formData.seoTitle} metaDescription={formData.metaDescription} keywords={formData.seoKeywords} content={formData.content} slug={formData.slug} featuredImageAlt={formData.featuredImageAlt} />
          </div>
        </div>
      </div>
    </div>;
