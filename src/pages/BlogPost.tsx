...
const BlogPost = () => {
  ...
  const loadPost = async (postSlug: string) => {
    const {
      data,
      error
    } = await supabase.from("blog_posts").select("*").eq("slug", postSlug).eq("status", "published").single();
    if (!error && data) {
      setPost(data);
      ...
      if (data.author_id) {
        ...
        const {
          data: profile
        } = await supabase.from("public_profiles").select("*").eq("id", data.author_id).single();
        if (profile) setAuthorProfile(profile);
      }
      if (data.tags && data.tags.length > 0) {
        const {
          data: related
        } = await supabase.from("blog_posts").select("id, title, slug, excerpt, featured_image_url, featured_image_alt").eq("status", "published").neq("id", data.id).limit(3);
  ...
  return <>
      ...
      <div className="min-h-screen flex flex-col bg-blog-background">
        ...
        <main className="flex-1">
          ...
          {post.featured_image_url && <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} transition={{
          ...
          <article className="container mx-auto px-4 py-16 md:py-24 max-w-[720px]">
            ...
            duration: 0.6
              ...
              <Link to="/blog" className="inline-flex items-center gap-2 text-blog-accent hover:text-blog-accent-hover mb-12 transition-colors text-base">
            ...
            delay: 0.2
              ...
              {/* Category Badge */}
              {post.tags && post.tags.length > 0 && <Badge variant="secondary" className="mb-6 bg-blog-badge-bg text-blog-accent font-medium border-0 px-4 py-1.5 text-sm">
                  {post.tags[0]}
                </Badge>}
              ...
              <div className="flex flex-wrap items-center gap-6 text-muted-foreground mb-10 pb-10 border-b border-blog-border">
                ...
                {post.read_time_minutes && <span className="flex items-center gap-2">
              ...
              {/* Excerpt */}
              {post.excerpt && <p className="text-xl md:text-2xl text-muted-foreground leading-relaxed">
                  {post.excerpt}
                </p>}
            </motion.header>
            ...
            delay: 0.4
                ...
                prose-blockquote:pl-8 prose-blockquote:italic prose-blockquote:text-muted-foreground prose-blockquote:my-12" dangerouslySetInnerHTML={{
            ...
            delay: 0.5
              ...
              <Button variant="outline" size="default" onClick={() => handleShare("linkedin")} className="gap-2 border-blog-border hover:border-blog-accent hover:text-blog-accent">
                <Linkedin className="h-4 w-4" />
                LinkedIn
              </Button>
              <Button variant="outline" size="default" onClick={() => handleShare("twitter")} className="gap-2 border-blog-border hover:border-blog-accent hover:text-blog-accent">
                <Twitter className="h-4 w-4" />
                Twitter
              </Button>
              <Button variant="outline" size="default" onClick={() => handleShare("copy")} className="gap-2 border-blog-border hover:border-blog-accent hover:text-blog-accent">
            ...
            {/* Related Posts */}
            {relatedPosts.length > 0 && <motion.section initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: 0.6
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {relatedPosts.map(related => <Link key={related.id} to={`/blog/${related.slug}`} className="group">
                      <Card className="h-full hover:shadow-blog-hover transition-all duration-300 overflow-hidden border-0 shadow-blog rounded-[20px]">
                        {related.featured_image_url && <div className="aspect-video overflow-hidden">
                            <img src={related.featured_image_url} alt={related.featured_image_alt || related.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                          </div>}
                        <CardContent className="p-5">
                          <h3 className="font-serif text-xl line-clamp-2 group-hover:text-blog-accent transition-colors mb-2">
                            {related.title}
                          </h3>
                          {related.excerpt && <p className="text-sm text-muted-foreground line-clamp-2">
                              {related.excerpt}
                            </p>}
                        </CardContent>
                      </Card>
